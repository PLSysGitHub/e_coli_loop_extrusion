...
def create_parser():
    parser = argparse.ArgumentParser(description='Simulations for tracking MSD of loci for short time intervals')
...
def out_folder_name(bacterium, args_in):
    if args_in.no_confinement:
        parent_folder="Results_3D/Short_times_no_confinement/"
    elif args_in.top_monomer!=1150:
        parent_folder=f"Results_3D/Short_times_start_top_monomer_{args_in.top_monomer}/"
    elif args_in.add_tether:
        parent_folder=f"Results_3D/Short_times_tether_monomer_{args_in.top_monomer}/"
    elif args_in.infinite_tube:
        parent_folder="Results_3D/Short_times_infinite_tube/"
    elif args_in.no_bypass:
        parent_folder="Results_3D/Short_times_no_bypass/"
    else:
        parent_folder="Results_3D/Short_times/"
...
def main():
    ...
    #to compare to diffusivity, we want to sample between 2 s and 500 s

    #1D simulation parameters
    simulation_time_min=500/60 #inferred_time(N)=replication of chromosome, essentially a generation
    ...
    delta_t_3D=1
    steps_per_sample=args.steps_per_sample*delta_t_sec #originally was for 1 s
    ...
    run_simulation(results_dir_1D, bacterium, rel_bond_wiggle, steps_per_sample, smcBondDist, 0.5*smcBondDist, save_folder,delta_t_3D, GPU_choice=GPU, F_z=args.force, col_rate=args.col_rate, trunc=args.trunc, top_monomer=args.top_monomer, infinite_tube=args.infinite_tube, no_confinement=args.no_confinement, add_tether=args.add_tether)
